let fs = require('fs');
let airportsMapping = JSON.parse(fs.readFileSync('assets/airports.json', 'utf8'));

const ICAO_HEADER = "ICAO";
...
(function() {
    ...
    app.get('/getAirport*', function(req,res,next) {
        console.log("Got request" + req);
        console.log(req.query);
        // Find based on ICAO, TODO- this is the current assumption maybe it will change when we get the data.
        let selected = null;
        for(let airport of airportsMapping) {
            console.log(airportsMapping);
            if (airport[ICAO_HEADER] == req.query[ICAO_HEADER]) {
